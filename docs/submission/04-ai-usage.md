...
| 단계            | 시작 신호                         | AI가 자동으로 수행                                                               | 사람이 직접 수행                          | 중단·인계 조건                        |
| ------------- | ----------------------------- | ------------------------------------------------------------------------- | ---------------------------------- | ------------------------------- |
| **1. 기능 정의**  | 아이디어 또는 칸반 카드                 | 관련 스펙·카드 검색, 기존 동작과 충돌 여부 확인                                              | 목적·우선순위·플레이 방식·제약 결정               | 결정이 부족하면 AI가 질문하고 대기            |
| **2. 작업 준비**  | 사람이 `/feature` 입력             | 최신 main 확인, `feature/<slug>` 생성, 수정 경로 선언, 카드 `doing` 이동, 시스템 스펙·Task 작성  | AI가 제시한 범위와 동작 확인                  | 작업 트리 오염·브랜치 충돌·범위 불명확 시 중단     |
| **3. 구현**     | 스펙과 Task 준비 완료                | Task 순서대로 코드·씬 수정, `docs/spec/` 동기화, API 조회, 테스트·Audit                    | 중간 빌드 플레이, 조작감·연출·밸런스 피드백          | 피드백 발생 시 스펙부터 고치고 구현 반복         |
| **4. 반영 승인**  | 구현·Audit 결과 보고                | 변경 파일과 남은 확인 항목 제시                                                        | 결과 확인 후 `/push` 입력                 | `/push` 전에는 AI가 커밋·머지하지 않음      |
| **5. 저장소 통합** | 사람이 `/push` 입력                | diff·스펙·Task·AC 대조, 카드 `review` 이동, 커밋 작성, `origin/main` 확인, main 머지·push | 충돌이나 범위 변경이 생기면 처리 방향 결정           | 스펙 불일치·검증 실패·원격 main 변경 시 자동 중단 |
| **6. 플레이 검증** | main 반영과 카드 `review`          | 없음                                                                        | 다른 팀원이 실제 플레이 후 `done` 또는 `fix` 결정 | 사람의 판정 전에는 완료 처리하지 않음           |
| **7. 상태 기록**  | 사람이 보드의 에이전트 프롬프트를 Cursor에 전달 | 지정 카드의 열·이력 갱신과 커밋                                                        | 반영 내용 확인                           | `fix`면 수정 흐름으로 복귀               |
...
AI는 위 판단을 대신하지 않고, 사람이 결정을 내린 사이의 브랜치 생성·문서화·구현·검사·기록을 자동화합니다.
